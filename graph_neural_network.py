...
class BorisGraphNet(nn.Module):
    def __init__(self, img_size=28, pred_edge=False):
        ...
        if pred_edge:
            ...
            A = self.precompute_adjacency_images(self,img_size)
            print("A*",A)
            self.register_buffer('A', A)


    @staticmethod
    def precompute_adjacency_images(self,img_size):
        col, row = np.meshgrid(np.arange(img_size), np.arange(img_size))
        ...
        print(A.shape)
        vol = float(A.sum())
        evals, D_rt_invU = self.approximate_laplacian(
                A, rank=256, which="LA"
            )
        deep_mat = self.deep_mat(
                evals, D_rt_invU, window=5, vol=vol, b=1
            )
        print("mejbah deepwalkmat size",deep_mat.shape)
        u, s, _ = sp.linalg.svds(deep_mat, img_size*img_size-1)
        self.embeddings = sp.diags(np.sqrt(s)).dot(u.T).T
        z = np.random.random((img_size*img_size,1))
        A = np.append(self.embeddings, z, axis=1)
        return torch.from_numpy((A)).float()

    def approximate_laplacian(self, A, rank, which="LA"):
        ...
        evals, evecs = sp2.eigsh(X, rank, which=which)
        print(
    ...
    def deep_mat(self, evals, D_rt_invU, window, vol, b):
        # approximate deepwalk matrix
        evals = self.dpwk_fltr(evals, window=window)
    ...
    def dpwk_fltr(self, evals, window):
...
if __name__ == '__main__':
    main()
# If you run # python mnist_fc.py --model graph you will get the adjacency matrix
# that I used
    # Examples:
